...
class Doc:
    ...
    def TBL(self, headers, rows, caption, key, col_w=None, font_sz=16, bold_rows=(), align_first_left=True, total=9360):
        ...
        def cell(text, w, bold=False, fill=None, align="left", white=False, keep=False):
            ...
            return (f'<w:tc><w:tcPr><w:tcW w:w="{w}" w:type="dxa"/>{shd}<w:vAlign w:val="center"/></w:tcPr>'
                    f'<w:p><w:pPr>{"<w:keepNext/>" if keep else ""}<w:jc w:val="{align}"/><w:spacing w:before="20" w:after="20"/>{rpr}</w:pPr>{content}</w:p></w:tc>')
        together = len(rows) <= 22      # a table of up to 22 rows stays on one page (keep-with-next on every row but the last)
        hdr = "".join(cell(h, col_w[i], bold=True, fill="1F5C99", align="center", white=True, keep=together) for i, h in enumerate(headers))
        hdr_row = f'<w:tr><w:trPr><w:tblHeader/><w:cantSplit/></w:trPr>{hdr}</w:tr>'
        body = ""
        for ri, r in enumerate(rows):
            fill = "EAF1F8" if ri % 2 else None; bd = ri in bold_rows
            cells = "".join(cell(v, col_w[i], bold=bd, fill=fill, align=("left" if (i == 0 and align_first_left) or ncol <= 2 else "left"),
                                 keep=together and ri < len(rows) - 1) for i, v in enumerate(r))
    ...
    def EQ(self, omml, key, params=None):
        ...
        n = self._eq
        eqp = ('<w:p><w:pPr><w:keepNext/><w:jc w:val="center"/><w:spacing w:before="60" w:after="60"/></w:pPr>'
               '<m:oMathPara xmlns:m="http://schemas.openxmlformats.org/officeDocument/2006/math"><m:oMath>' + omml + '</m:oMath></m:oMathPara></w:p>')
        nump = f'<w:p><w:pPr><w:keepNext/><w:jc w:val="right"/></w:pPr>{run(f"({n})")}</w:p>'
        # the sentence that introduces the equation stays on the same page as the equation
        if (self.xml and self.xml[-1].startswith('<w:p><w:pPr>') and '<w:keepNext/>' not in self.xml[-1]
                and '<w:ind w:left="567"/>' not in self.xml[-1]):      # not a symbol line of the previous equation: that would chain equations
            self.xml[-1] = self.xml[-1].replace('<w:pPr>', '<w:pPr><w:keepNext/>', 1)
        ...
        self.add(tbl)
        if params:
            self.add(f'<w:p><w:pPr><w:pStyle w:val="BodyText"/><w:keepNext/><w:spacing w:after="40" w:line="240" w:lineRule="auto"/></w:pPr>{run("where:")}</w:p>')
    ...
    def LANDSCAPE(self, fn):
        ...
        Two landscape blocks with nothing between them are merged into one section, otherwise the empty portrait
        section between them prints as a blank page."""
        if getattr(self, "_land_close", None) == len(self.xml):
            self.xml.pop()                                     # drop the previous block's closing break; stay landscape
        else:
            self.add(f'<w:p><w:pPr>{self._sectpr(False)}</w:pPr></w:p>')
        fn(self)
        self.add(f'<w:p><w:pPr>{self._sectpr(True)}</w:pPr></w:p>')
        self._land_close = len(self.xml)
...
def patch_styles():
    """W2 (user review 2026-09-23): headings flush left (no hanging indent, a space after the number instead of a tab), bullets in the body
    size with a small hanging indent and a little air between items. Style-level edits, so every heading and bullet in the document follows."""
    np_ = os.path.join(WORK, "word", "numbering.xml"); nx = open(np_, encoding="utf-8").read()
    def fix_level(m):
        body = m.group(0)
        if "Heading" not in body: return body
        body = re.sub(r"<w:ind [^>]*/>", '<w:ind w:left="0" w:firstLine="0"/>', body)
    ...
    if m:
        ...
        def fix_bullet(mm):
            body = mm.group(0)
            return re.sub(r'(<w:lvl w:ilvl="0"[^>]*>.*?)<w:ind [^>]*/>', r'\1<w:ind w:left="357" w:hanging="357"/>', body, count=1, flags=re.S)
        nx = re.sub(r'<w:abstractNum w:abstractNumId="%s"[^>]*>.*?</w:abstractNum>' % aid, fix_bullet, nx, flags=re.S)
    open(np_, "w", encoding="utf-8").write(nx)
    sp = os.path.join(WORK, "word", "styles.xml"); sx = open(sp, encoding="utf-8").read()
...
if __name__ == "__main__":
    ...
    elif "--results" in sys.argv: build_results()
    else: build()
